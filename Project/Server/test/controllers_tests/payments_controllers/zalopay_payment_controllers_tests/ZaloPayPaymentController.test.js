...
describe("Ctrlr Tạo đơn hàng", () => {
  ...
  test("Thiếu successUrl - 400", async () => {
    ...
    expect(processorMock.checkValidateUrl).toBeCalledTimes(1);
...
describe("Ctrlr Lưu đơn hàng đã thanh toán", () => {
  ...
  test("Thông tin của zalo bị đổi - 400", async () => {
    //Arrange
    const id = 1;
    const controller = getController();
  ...
  test("Order không tồn tại - 404", async () => {
    ...
    const reqMock = {
      params: { id },
      query: { successUrl: "wtf", cancelUrl: "wtf", data: {} },
    };
    const resMock = new ResponseMock();
