...
const ProductBox = ({ id }) => {
    ...
    useEffect(() => {
        (async () => {
            let data = await ProductServices.getProduct(id, 'prod_no', 'prod_imgs', 'prod_name', 'prod_details', 'prod_hardwareAndOS')
            ...
            <div className="ProductBox">
                    ...
                    <div className="box-silder-wrapper">
                        ...
                        <ul>
                            {images && images.map((item, index) =>
                                <li key={index}
                                    onMouseEnter={() => setPos(index)}>
                                    <img style={pos === index ? { opacity: '1' } : null} alt="imgae.jpg" src={item} />
                                </li>)}
                        </ul>
                    </div>
                </div>
                <div className="product-box-detail">
                    ...
                    <div className="class-price">
                        {product && product.prod_details.map((item, index) =>
                    ...
                    <div className="product-box-notify">
                        ...
                        <p>Đổi trả sản phẩm trong 15 ngày.<br />Bảo hàng theo chính sách bảo hành chính hãng!</p>
                    </div>
                    {
                        product && <>
                            {product.prod_details[choose].pd_amount=== product.prod_details[choose].pd_sold?
                            <button style={{background:'transparent',color:'var(--backgroundColor)', border:'2px solid var(--backgroundColor)'}} >hết hàng</button>
