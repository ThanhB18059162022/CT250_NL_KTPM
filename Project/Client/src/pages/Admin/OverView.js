        ...
        (async () => {
            ...
            setPaidedOrders(
                data.map((item) => ({
                    ...
                    customer: { ...item.customer, cus_address: item.customer.cus_location },
                }))
            );
        })();
    }, [setPaidedOrders]);
                    ...
                    <ul className='OverView_Body'>
                        {orders.length > 0 ? (
                                ...
                                {orders.map((order, index) => (
                                    <li key={index}>
                                        ...
                                        <p className='orderphone'>
                                            {order.customer.cus_phoneNumber}
                                        </p>
                                        ...
                                        <p>
                                            ...
                                            {!order.paid && (
                                                <button
                                                    onClick={() =>
                                                        onDeny(order.id, order.customer.cus_name)
                                                    }
                                                >
                            ...
                            <p style={{ textAlign: "center", padding: "10px 0" }}>
                                Đơn hàng đặt tại nhà trống
                            </p>
                        )}
                    </ul>
                </div>
                <div className='OverViewList' style={{ marginTop: "20px" }}>
                    <h3 style={{ marginBottom: "10px", color: "var(--backgroundColor)" }}>
                    ...
                    <div className='OverViewList_Header'>
                        ...
                        <p className='orderemail'>Email</p>
                        <p className='orderphone'>Số điện thoại</p>
                        <p className='totalorder'>Thanh toán</p>
                        <p className='totalorder'>Giá Trị Đơn</p>
                        <p className='timecreated'>Thời gian thanh toán</p>
                        <p>Hành động</p>
                    </div>
                    <ul className='OverView_Body' style={{ maxHeight: "100vh" }}>
                        {paidedOrders.length > 0 ? (
                            <>
                                {paidedOrders.map((order, index) => (
                                    <li key={index}>
                                        ...
                                        <p>
                                            <button
                                                onClick={() => setDetail({ ...order, paid: true })}
                                            >
                                                <FontAwesomeIcon icon={faEye} />
                                            </button>
                                        </p>
    ...
    return (
        <div className='detail_transaction'>
            <div className='detail_wrapper'>
                ...
                {!paid && (
                    <div className='order_behavior'>
                        <button onClick={() => onAccept(detail.id, customer.cus_name)}>
                            Xác nhận thanh toán
                        </button>
                        <button onClick={() => onDeny(detail.id, customer.cus_name)}>
                            Hủy đơn hàng
                        </button>
                    </div>
                )}
